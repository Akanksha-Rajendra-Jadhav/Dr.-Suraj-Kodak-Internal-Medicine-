 ...
 try{
  ...
  if(reviews){
    ...
    let reviewTimer=setInterval(()=>{
      const card=reviews.querySelector("article");
      if(!card) return;
      const max=reviews.scrollWidth-reviews.clientWidth;
      const next=reviews.scrollLeft+reviews.clientWidth;
      reviews.scrollTo({left:next>max+5?0:next,behavior:"smooth"});
    },5000);
    reviews.addEventListener("mouseenter",()=>clearInterval(reviewTimer));
    reviews.addEventListener("mouseleave",()=>reviewTimer=setInterval(()=>{
      const max=reviews.scrollWidth-reviews.clientWidth;
      const next=reviews.scrollLeft+reviews.clientWidth;
      reviews.scrollTo({left:next>max+5?0:next,behavior:"smooth"});
    },5000));
...
/* ===== STICKY NAVBAR + VISIBLE SCROLL DIRECTION EFFECT ===== */
(function(){
...
/* ===== GUARANTEED FIXED/STICKY NAVBAR HEIGHT ===== */
(function(){
  const header=document.querySelector('.header');
  if(!header) return;
